...
{{--                    {{ $item->created_at }}</td>--}}
                ...
                <td
                    ...
                    @can('會員管理_修改')
                        @if($item->line_id)
                            <form method="post" action="{{route('bind_line_members.update',["member"=>$item->id])}}">
                                @csrf
                                @method("PUT")
                                <input type="hidden" name="line_id" class="line_id">
                                <button type="submit" class="btn btn-sm btn-danger" onclick="document.getElementById('line_id').value = document.getElementById('head_line_id').value;">覆蓋原綁定</button>
                            </form>
                        @else
                            <form method="post" action="{{route('bind_line_members.update',["member"=>$item->id])}}">
                                @csrf
                                @method("PUT")
                                <input type="hidden" name="line_id" class="line_id">
                                <button type="submit" class="btn btn-sm btn-primary" onclick="document.getElementById('line_id').value = document.getElementById('head_line_id').value;">新綁定</button>
                            </form>
                        @endif
