...
{{--                </div>--}}
    ...
    <table class="w-full">
        ...
        <tr>
            <th
                wire:click="sortBy('name')" :sortDirection="$sortByColumn=='line_id'?$sortByDirection:null"
                class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-center">
                LINE帳號
                <x-table-sort-icon :sortByColumn="$sortByColumn" :sortByDirection="$sortByDirection" column="line_id" />
            </th>
            <th
            ...
            <th
                wire:click="sortBy('member_line_id')" :sortDirection="$sortByColumn=='member_line_id'?$sortByDirection:null"
                class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-center">
                會員
                <x-table-sort-icon :sortByColumn="$sortByColumn" :sortByDirection="$sortByDirection" column="member_line_id"  />
            </th>
            <th
        ...
        @foreach ($paginator->items() as $item)
            <tr>
                ...
                <td
                    ...
                    @can('LINE對話記錄管理_修改')
                        <a class="btn btn-sm btn-primary" href="{{route('line_messages.edit',["line_message"=>$item->id])}}">編輯</a>
                    @endcan
                </td>
            </tr>
        @endforeach
